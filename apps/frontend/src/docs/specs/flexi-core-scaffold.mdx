...
<Meta title="Specs/Platform & Tooling/Flexi Core Scaffold" />
...
- `README.md` (setup/run) and `ROADMAP.md` (the planning documents that existed at scaffold time).
...
- `pnpm --filter backend test:e2e` — historical expectation: all 11 stub routes plus `/api/health` returned the expected envelope when this scaffold record was accepted.
