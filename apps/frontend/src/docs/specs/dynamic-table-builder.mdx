...
At the time this SPEC was written, Flexi's roadmap split off a Dynamic Table Builder as an independently shippable module and the platform only carried its supporting tenant-schema-routing layer (`TenantKnexService`, `resolveTenantSchema()`). The backend now supports defining and using dynamic structures through its API; the tenant-admin builder UI remains unfinished. This record captures the original goal: let a tenant admin define, evolve and populate tables and relations on real Postgres tables scoped to that tenant's schema, without a developer or code deploy.
