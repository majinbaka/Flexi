...
<Meta title="Specs/Platform & Tooling/Figma Design Sync (SPEC)" />
...
- [x] `docs/process/figma-sync-ledger.md` -- new file with a table header (`Figma node | node-id | Frontend path | Status | Last synced | Notes`) and zero data rows -- tracks gradual whole-file sync progress.
