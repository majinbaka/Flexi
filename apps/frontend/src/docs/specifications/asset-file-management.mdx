...
Các path bên dưới giữ nguyên contract đầu vào. ADR-003 đã chốt base path là
`/api/v1`, nên các path `/api/v1/...` dưới đây là đúng convention; backend còn
dùng global prefix `/api` cho tới khi Story 0.2.1 (#76) chuyển. ADR-004 đã chốt
response envelope: giữ `{ success, data, error }`, error code
SCREAMING_SNAKE_CASE không prefix, một code ứng đúng một HTTP status.
...
<DocumentationTable
  ...
  rows={[
    ...
    {
      id: 'versioning',
      decision: 'Base path và response envelope nào?',
      proposal:
        'Đã chốt: base path /api/v1 (ADR-003, chuyển ở #76) và envelope { success, data, error } (ADR-004). Error code đăng ký ở registry #78.',
