...
Vì API versioning và error envelope đang mâu thuẫn, tài liệu hợp nhất dùng
`{apiBase}`. ADR phải ánh xạ nó tới `/api` hoặc `/api/v1` một lần cho toàn nền
tảng; không module nào tự chọn prefix.
