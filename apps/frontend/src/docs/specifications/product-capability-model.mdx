...
Ba mâu thuẫn có ảnh hưởng kiến trúc từng chặn việc mở Epic nền tảng đều đã
được chốt: Environment thuộc Application (ADR-001), topology tenant × environment
(ADR-002), và API/error contract giữ envelope hiện tại thay vì Problem Details
(ADR-004). Trang **Decisions & Risks** giữ trạng thái đầy đủ của Decision Log;
những mục còn Open ở đó vẫn không được tự động giải quyết trong tài liệu này.
...
<DocumentationTable
  ...
  rows={[
    ...
    },
    {
      ...
      rule: 'Envelope { success, data, error } và prefix /api/v1 đã chốt (ADR-003, ADR-004); correlationId/retryable chưa lên wire; không lộ cross-tenant existence.',
    },
    {
      id: 'release',
...
code sang `/api/v1` nằm ở Story 0.2.1 (#76). ADR-004 đã chốt error envelope:
giữ `{ success, data, error }`, code `SCREAMING_SNAKE_CASE` không prefix `ERR_`,
và một code ứng đúng một HTTP status.
...
Các alias `pages:*` hiện có trong spec nguồn chưa phải canonical production
contract; migration map của chúng chờ ADR-005 (permission grammar). Với error
code thì ADR-004 đã chốt: không dùng prefix `ERR_`, và Problem Details không lên
wire — mọi `ERR_*` còn trong spec thuộc module chưa viết và được đổi tên khi
registry #78 ra đời.
