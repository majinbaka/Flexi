...
flowchart TD
  ...
  G --> H[Trả 409 APP_IMPORT_SLUG_CONFLICT]
...
Payload trên là Problem Details contract của endpoint mục tiêu. Backend hiện
dùng global envelope `{ success, data, error }`; task implementation phải chốt
cách nhúng Problem Details vào envelope hoặc ngoại lệ endpoint trước khi code,
không được âm thầm trả một shape khác.
...
- **Then** hệ thống trả `409 Conflict` với code
  `APP_IMPORT_SLUG_CONFLICT`.
...
3. Kiểm tra uniqueness trong tenant và map conflict sang Problem Details 409.
...
`APP_IMPORT_SLUG_CONFLICT`; pre-check một mình không đủ đảm bảo atomicity.
...
<DocumentationTable
  ...
  rows={[
    ...
    },
    {
      ...
      evidence:
        'Không có Application/FAPS DTO, target_slug hoặc APP_IMPORT_SLUG_CONFLICT type/code.',
    },
    {
      id: 'frontend',
      surface: 'Frontend',
