  ...
  rows={[
    {
      ...
      exit: 'ADR-001…008 approved; contract owners và compatibility policy được chỉ định. Đã có: ADR-001 (Environment thuộc Application, MVP một environment mặc định), ADR-002 (topology Hybrid — PROD tách cluster, DEV/STG chung), ADR-003 (base path /api/v1, compatibility policy breaking/additive, owner là vai trò API Governance — người cụ thể còn phải gán).',
    },
    {
      id: 'mvp-a',
