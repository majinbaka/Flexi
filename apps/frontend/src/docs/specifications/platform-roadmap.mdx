...
5. Multi-environment promotion, approval và HA chỉ mở rộng khi single-scope
   release/rollback đã deterministic.
  ...
  rows={[
    {
      ...
      exit: 'ADR-001…008 approved (ADR-007 đã Accepted, xem docs/adr/); contract owners và compatibility policy được chỉ định.',
    },
    {
      id: 'mvp-a',
      ...
      capabilities:
        'Tenant/IAM hardening; Application; default scope; Configuration/SecretRef; Artifact Registry; Audit baseline.',
      entry: 'Gate 0.',
...
- **Story 2.1.1:** Định nghĩa catalog schema, override scope và effective-value
  precedence sau ADR Environment.
...
- **Story 3.1.1:** Chuẩn hóa ResourceRef, schemaVersion, checksum và optimistic
  concurrency token trong `packages/shared-types/src/contracts/`, theo ADR-007
  (`docs/adr/ADR-007-artifact-contract-location.md`): token là integer version,
  checksum chỉ dùng cho toàn vẹn revision.
...
- **Story 11.1.1:** Provision topology đã được ADR chốt và bind config/secrets riêng.
...
- Không triển khai canvas Page/Workflow bằng schema riêng lẻ trước common
  ResourceRef, revision và dependency contract. ADR-007 đã chốt contract này
  nằm ở `packages/shared-types/src/contracts/`; Epic 6 và Epic 7 import từ đó
  thay vì tự định nghĩa.
