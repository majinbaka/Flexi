...
Response envelope theo ADR-004 (`{ success, data, error }`), và error code ở
bảng dưới sẽ bỏ prefix `ERR_` khi module được viết — registry #78 giữ spelling
cũ làm alias. Authentication, authorization, DTO validation và error mapping của
target endpoints vẫn là **Not confirmed from current implementation**.
