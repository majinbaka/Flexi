...
<div className="grid grid-cols-1 gap-md md:grid-cols-3">
  ...
  <Card>
    <div className="flex flex-col gap-sm">
      ...
      <p className="font-body-sm text-body-sm text-on-surface-variant">
        Tenant switch, MFA, account lockout, service account, RLS/FLS, SSO,
        SCIM, impersonation và JIT access.
      </p>
    </div>
  </Card>
</div>
  ...
  rows={[
    {
      ...
      gap: 'Thiết kế đề xuất Global User dùng chung identity giữa nhiều tenant; repository hiện dùng identity độc lập theo actor/tenant.',
    },
    {
      id: 'session',
      area: 'Session',
    ...
    },
    {
      ...
      gap: 'Chưa có switch-tenant endpoint hoặc mã lỗi ERR_TENANT_MISMATCH theo contract đề xuất.',
    },
    {
      id: 'audit',
      area: 'Audit',
...
<DocumentationTable
  ...
  rows={[
    ...
    },
    {
      id: 'mvp-tenant',
      phase: 'MVP',
      capability: 'Tenant isolation và tenant switch.',
      current: 'Isolation đã có cho Dynamic Tables; tenant switch chưa có.',
    },
    {
      id: 'mvp-onboarding',
      phase: 'MVP',
    ...
    },
    {
      ...
      current:
        'JWT payload dành trước impersonatedBy nhưng không gán; các capability khác chưa có.',
    },
  ]}
/>
...
<DocumentationTable
  ...
  rows={[
    ...
    },
    {
      ...
      current: 'Chưa có endpoint, policy, token hoặc audit flow.',
    },
    {
      id: 'switch',
      rule: 'Tenant switch',
      target:
        'Mỗi JWT có một active tenant; POST /auth/switch-tenant phát token mới.',
      current:
        'Mỗi tenant token đã có một tenantId, nhưng user không có membership đa tenant trên cùng identity và không có switch API.',
    },
    {
      id: 'transfer',
  ...
  rows={[
    {
      ...
      purpose: 'Global identity và membership tenant.',
      current:
        'Có cùng tên nhưng TenantUser trỏ tới AuthAccount độc lập; không phải mapping một global user sang nhiều tenant.',
    },
    {
      id: 'tenant',
      target: 'Tenant',
...
1. **Core IAM:** chốt identity model và permission taxonomy; bổ sung membership
   đa tenant, tenant switch, Users/Role APIs và invariant last owner.
  ...
  rows={[
    {
      ...
      options:
        'Global User + TenantUser membership hoặc identity độc lập theo tenant.',
      evidence:
        'Code hiện tại là identity độc lập. Global identity là thay đổi data model, login resolution và refresh-token ownership.',
    },
    {
      id: 'rls',
...
<DocumentationTable
  ...
  rows={[
    ...
    },
    {
      id: 'missing',
      responsibility:
        'Switch tenant, Users/Role management, MFA, service account, RLS/FLS, SSO/SCIM, impersonation và JIT',
