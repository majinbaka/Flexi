...
1. **Tenant-customizable _fields_ (not full tables)** via JSONB columns, EAV/metadata-config tables, or a single generic "custom data" column — the common pattern for letting tenants add custom fields without DDL changes. This is explicitly a workaround that _avoids_ per-tenant DDL/dynamic table creation, per multiple aggregated sources (Medium/dev.to-level content, not individually deep-verified — confidence low).
2. **General schema-per-tenant catalog bloat** (Q3 sources above) — which is directly relevant by extension/inference but was not written about in the specific context of _user-triggered_ table creation.
