...
- **claim**: Recommended mitigations reported in these threads/write-ups: (a) use `track_extra_parameters` in PgBouncer so it tracks and restores specific session-level parameters (e.g. `search_path`) per client across backend swaps; (b) set the default at the _role_ level via `ALTER ROLE <user> SET search_path = <schema>` rather than per-session `SET`; (c) replace every session-scoped `SET` with `SET LOCAL` (or `set_config(..., true)`) issued _inside every transaction_, since `SET LOCAL` is transaction-scoped and therefore safe under transaction-mode pooling — this was described as "the fix" for the search_path-persisting-on-a-recycled-backend root cause.
