...
- source_spec: `apps/frontend/src/docs/specs/flexi-core-scaffold.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/lint-ci-tooling.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/core-authentication.mdx`
...
- source_spec: none
  ...
  evidence: Independently shippable deliverable split out of the schema-per-tenant implementation guide (`docs/research/schema-per-tenant-implementation-guide.md`, §5) per multi-goal check in step-01. Depends on the tenant context & schema-routing core landing first.
...
- source_spec: none
  ...
  evidence: Independently shippable deliverable split out of the schema-per-tenant implementation guide (`docs/research/schema-per-tenant-implementation-guide.md`, §7) per multi-goal check in step-01. The guide stresses this is load-bearing, not optional polish -- no external source validates safe ceilings for schema-per-tenant combined with uncapped tenant-created dynamic tables, so the caps are Flexi's own starting risk mitigation pending real usage data.

- source_spec: `apps/frontend/src/docs/specs/schema-per-tenant-core.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/core-authentication-frontend.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/admin-login.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/refresh-token-reuse-detection.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/editorconfig-drift-check.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/deferred-fixes-batch.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/deferred-fixes-batch-2.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/dynamic-table-builder-story-2-table-field-crud.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/dynamic-table-builder-story-3-row-readwrite-api.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/dynamic-table-builder-story-4-linked-record-fields.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/onboarding-1-1-entry-and-form-shell.mdx`
  ...
  evidence: Surfaced by Step 4 blind-hunter review against the full dirty-worktree diff. `docs/design/ux-experience.md` appears to allow a manual cleanup action, while the PRD/epic requirements say failed attempts are view-only in Phase 1. This predates Story 1.1 implementation and belongs in planning-contract cleanup.

- source_spec: `apps/frontend/src/docs/specs/onboarding-1-1-entry-and-form-shell.mdx`
  summary: Choose one canonical setup secret handoff contract: setup link versus raw setup token response.
  evidence: Surfaced by Step 4 blind-hunter review. `apps/frontend/src/docs/specs/super-admin-tenant-onboarding.mdx` describes raw `setupToken` response fields while PRD/UX requirements emphasize Setup Link handoff. Story 1.1 does not implement setup handoff, so this should be resolved before Epic 2/3 setup-link stories.
...
- source_spec: `apps/frontend/src/docs/specs/onboarding-1-4-idempotent-submission-handling.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/onboarding-2-2-tenant-schema-provisioning-and-bootstrap.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/onboarding-2-3-bootstrap-defaults-and-rbac-seed.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/onboarding-2-5-setup-link-generation-and-backup-email.mdx`
...
- source_spec: `apps/frontend/src/docs/specs/onboarding-2-6-activation-boundary-failure-and-audit.mdx`
  summary: `TenantProvisioningWorker.withTimeout()`'s `Promise.race` never cancels or awaits the losing `startLifecycle()` promise, so a timeout-triggered `recordProvisioningTimeout()` (which now runs real compensation -- schema drop, actor deletion, `Tenant.status = FAILED`) can execute concurrently with a still-running provisioning pass on the same attempt/tenant.
  evidence: Blind-hunter review of Story 2.6. The race mechanism itself (`Promise.race` without cancellation) predates this story (`provisioning.worker.ts`, unchanged by this diff), but Story 2.6 is what makes the consequence severe -- previously a timeout only flipped a cheap `stepOutcomes` flag; now it can concurrently drop a schema and delete actor rows out from under an in-flight success path. `activation()`'s missing current-status guard (patched in this review loop) narrows but does not close this window.

- source_spec: `apps/frontend/src/docs/specs/onboarding-2-6-activation-boundary-failure-and-audit.mdx`
  summary: `FirstAdminService.deactivate()`'s `TenantUser` lookup (`tenantUser.findFirst({ where: { tenantId } })`) has no ordering or role filter, so if a tenant ever has more than one `TenantUser` row, compensation nondeterministically deletes only one and silently leaves others behind.
  evidence: Blind-hunter and edge-case-hunter review of Story 2.6 both found this independently. Currently unreachable in practice -- Story 2.4's `FirstAdminService.assign()` is the only writer and is itself idempotent/single-row (check-before-create) -- but nothing enforces the single-row invariant at the schema level, so a future Epic 3 feature adding more `TenantUser` rows before a failure path fires would hit this silently.

- source_spec: `apps/frontend/src/docs/specs/onboarding-2-6-activation-boundary-failure-and-audit.mdx`
  summary: No test covers `finalizeAudit()` itself throwing/failing after `activation()` has already succeeded -- only the compensation-sub-step-failure path is tested for audit-write failure, leaving the "tenant is ACTIVE but the permanent audit row never got written" state unverified.
  evidence: Blind-hunter review of Story 2.6. `finalizeAudit()` is already double-guarded (try/catch, never rethrows) per its own doc comment, so this is a coverage gap rather than a code defect -- worth closing in a future test pass rather than blocking this story's review loop.
